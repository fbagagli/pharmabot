...
def test_scrape_basket_logic(session: Session):
    ...
    with patch("pharmabot.services.scraper.scrape_product", return_value=mock_offers) as mock_scrape:
        # 3. Run the function
        scrape_basket(session)

        # 4. Verify interactions
        mock_scrape.assert_called_once_with(minsan="987654321", headless=True)
