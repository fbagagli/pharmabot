...
def refresh_table(table: ui.table) -> None:
    ...
    with database.get_session() as session:
        ...
        table.rows = [
            {"id": p.id, "minsan": p.minsan, "name": p.name}
            for p in products
        ]
        table.update()
        ...
        def save():
            ...
            with database.get_session() as session:
                try:
                    ...
                    ui.notify(f"Product updated.", type="positive")
                    refresh_table(table)
                    dialog.close()
                except Exception as e:
                    ui.notify(f"Error: {e}", type="negative")
        ...
        def delete():
            with database.get_session() as session:
                try:
                    ...
                    ui.notify(f"Product deleted.", type="positive")
                    refresh_table(table)
                    dialog.close()
                except Exception as e:
                    ui.notify(f"Error: {e}", type="negative")
...
def render() -> None:
    ...
    columns = [
        ...
        {'name': 'minsan', 'label': 'Minsan', 'field': 'minsan', 'align': 'left', 'sortable': True},
